<template>
  <div class="classic-page">
    ...
    <div class="container">
      <div class="side">
        <div class="form">
          <div class="control">
            ...
            <div class="button">
              ...
              <div class="g-clickable icon" @click="isChainSetting = true">
                <img
                  class="g-dark-only"
                  :src="chain.iconDark"
                  alt=""
        ...
        >
          <Setting
            @change="quote"
            @back="isShowSetting = false"
            v-if="isShowSetting"
          />
        </NouDialog>
        <NouDialog
          close-on-click-modal
          :visible.sync="isChainSetting"
          :dialog-class="advanceContentClasses"
          :scrollDomID="'dialog-scroll-dom'"
        >
          <ChainView @close="isChainSetting = false"></ChainView>
        </NouDialog>
        <SelectToken
          :is-show="isShow"
          :show-native-token="tabCode === '01'"
          :dom="dom"
          @select="changeToken"
          @close="hideSelectToken"
          @changeDom="changeDom"
        />
      </div>
    </div>
    ...
    <BlurMask
      :visible.sync="isH5TransactionShowing"
      :scrollDomId="'action-sheet-scroll'"
    >
...
import getChains, { isV2 } from "@/utils/getChains";
...
export default {
  ...
  data() {
    const walletType = window.localStorage.getItem("walletType") || "bsc";
    return {
      isV2: isV2(walletType),
  ...
  computed: {
    ...
    pairCode() {
      if (this.tabCode === "01") {
        return this.tokenSwapPrev.symbol + "/" + this.tokenSwapNext.symbol;
      }
      if (this.tabCode === "03") {
        return this.tokenSwapPrev.symbol + "/" + this.tokenSwapNext.symbol;
      }
    ...
    tokenPrev() {
      if (this.tabCode === "03") {
        return this.tokenSwapPrev;
      }
      return this.tabCode === "01" ? this.tokenSwapPrev : this.tokenLimitPrev;
    },
    tokenNext() {
      if (this.tabCode === "03") {
        return this.tokenSwapNext;
      }
      return this.tabCode === "01" ? this.tokenSwapNext : this.tokenLimitNext;
    },
    isFavorite() {
      const list = this.favoriteList.filter((item) => {
        return item === this.pairCode;
      });
    ...
    chain() {
      ...
      const temp = list.filter((item) => {
        return item.value === this.walletType;
      });
  ...
  watch: {
    ...
    $route: {
      handler: function (val, oldVal) {
        // first time enter
        if (!oldVal && this.tokenList.length === 0) {
          setTimeout(() => {
            this.changeByPair(val,1);
          }, 300);
        } else {
          this.changeByPair(val);
        }
  ...
  methods: {
    ...
    changeByPair(val) {
      ...
      if (!params) return;
      const fromToken = this.findTokenBySymbol(params[1]);
      const toToken = this.findTokenBySymbol(params[2]);
      if (fromToken && toToken) {
        this.changeLocalToken(fromToken, true);
        this.changeLocalToken(toToken);
      }
    },
    changeConfig(val) {
      const { query } = val || {};
    ...
    findTokenBySymbol(symbol) {
      ...
      const result = this.tokenList.filter(item => {
        return item.symbol === symbol;
      });
      return result && result[0];
    },
    changeLocalToken(target, from) {
    ...
    async loadToken() {
      let chain = getCache(CACHE_KEY.Chain);
      if (isCoin98()) {
        let appChain = await getAppWalletType();
        chain = appChain ? appChain : chain;
      }
      this.getToken(chain);
    },
  },
...
<style lang="scss">
body {
  background-color: var(--background-classic-body);
}
</style>
