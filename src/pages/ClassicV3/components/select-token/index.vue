  ...
  >
    <div v-if="isAddToken" class="add-token" id="dialog-scroll-dom">
      ...
      <div class="add-token__foot">
        ...
        >
          <rect width="18" height="18" rx="3" fill="var(--classic-button-disabled)" />
        </svg>
    ...
    <div v-else class="select-token">
      ...
      <div class="favorites-pairs" v-if="tabCode === '02'" :id="dom">
        <div
          class="item"
        ...
        >
          ...
          <div class="item__text">
            ...
            <!-- <div class="item__name">{{ item.name }}</div> -->
            <div class="item__name">{{ getFavoriteName(item) }}</div>
          </div>
          <div class="item__delete" @click.stop="removeFavorite(item)">
...
import { contract as state, loadDefaultToken, loadLimitDefaultToken } from "@/contract";

export default {
  name: "SelectToken",
  components: { NouDialog, SearchInput, ToolTip },
  props: {
    isShow: {
      type: Boolean,
      default: false,
  ...
  computed: {
    isCustomToken() {
      return isCustomToken(this.walletType);
    },
    popularToken() {
      ...
      return this.tokenList.filter((item) => {
        return item.hot === "01" && !isNativeToken(this.walletType, item.address);
      });
    },
    allToken() {
      if (!this.query) {
    ...
    filterToken(){
      ...
      return this.allToken.filter(item=>{
        return !isNativeToken(this.walletType, item.address);
      })
    },
    getCustomList() {
  ...
  methods: {
    ...
    getFavoriteName(pairCode) {
      const [symbol_from, symbol_to] = pairCode.split("/");
      const token_from =
        this.tokenList.find((token) => token.symbol === symbol_from) || {};
      const token_to =
        this.tokenList.find((token) => token.symbol === symbol_to) || {};
      return (
        (token_from.name || symbol_from) + " / " + (token_to.name || symbol_to)
      );
    },
    selectFavorite(pairCode) {
      const [symbol_from, symbol_to] = pairCode.split("/");
      const target_from = this.tokenList.find(
        (token) => token.symbol === symbol_from
      );
      const target_to = this.tokenList.find(
        (token) => token.symbol === symbol_to
      );

      const getToken = (target, from) => {
        const { code, name, symbol, address, icon, decimals } = target;
        const token = {
          id: code,
          name,
          symbol,
      ...
      const token_from = getToken(target_from, true);
      const token_to = getToken(target_to, false);
      Object.assign(state.tokenSwapList[this.tokenSwap], token_from);
      Object.assign(state.tokenSwapList[1 - this.tokenSwap], token_to);
    ...
    removeToken(item) {
      ...
      if (state.tokenSwapList[0].address === address || state.tokenSwapList[1].address === address) {
    ...
    async searchToken(e, address) {
      ...
      const res = await getTokenInfo(this.walletType, this.address);
      if (res && res.name) {
        this.isInvalidToken = false;
