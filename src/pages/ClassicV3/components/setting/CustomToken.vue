...
export default {
  ...
  methods: {
    addToken() {
      ...
      window.localStorage.setItem(this.walletType + '_customtoken', JSON.stringify(this.myTokenList));
    ...
    removeToken(item) {
      ...
      window.localStorage.setItem(this.walletType + '_customtoken', JSON.stringify(list));
      this.$emit('removetoken', item);
    },
    getTokenList() {
      const temp = window.localStorage.getItem(this.walletType + '_customtoken');
...
.select-token__info {
  ...
  justify-content: space-between;
  margin-bottom: 10px;
  background-color: var(--background-select-token-info);
  padding: 10px;
  box-sizing: border-box;
  border-radius: 6px;
