<template>
  <div class="bottom">
    ...
    <div class="connect">
      ...
      <div
        v-else
        class="c-user__content dex-user__connect g-font-mid"
        @click="connect"
      >
...
import { tryConnect, connect, changeAccount, disConnect } from "@/init";
...
export default {
  ...
  methods: {
    ...
    async connect() {
      if (!isApp() && isMobile()) {
        console.log(this);
        const sdk = await tryConnect(this.walletType, "WalletConnect");
        if (sdk) {
          await connect(sdk);
          await changeAccount(
            sdk.wallet.address,
    ...
    showMenu() {
      ...
      if (isLimitOrder(this.walletType)) {
        menus.push({
          label: "Limit Orders",
          action: () => {
            try {
      ...
      if (!isApp() && isMobile()) {
        menus.push({
          ...
          action: () => {
            this.totalDisconnect(this.walletType);
          },
        });
      }
...
.bottom {
  ...
  .theme-wrapper {
    width: 40px;
    height: 40px;
    background: var(--background-header-chain);
    display: flex;
